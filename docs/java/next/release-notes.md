...
## Should I update?[​](#should-i-update "Direct link to Should I update?")
...
## 2.0.0-M1 - September 25, 2026[​](#200-m1---september-25-2026 "Direct link to 2.0.0-M1 - September 25, 2026")

[All Release Changes](https://github.com/SAP/ai-sdk-java/releases/tag/rel%2F2.0.0-M1)
...
* \[Prompt Registry] `OrchestrationConfigClient` and `PromptClient` were reworked into unified `PromptRegistryClient`, see migration guide below for additional information
...
* \[Orchestration] Internal AI SDK methods `createInputFilterConfig` and `createOutputFilterConfig` were removed from public API
...
#### Prompt registry client Migration Guide[​](#prompt-registry-client-migration-guide "Direct link to Prompt registry client Migration Guide")
...
#### Spring AI 2.0.1 Migration Guide[​](#spring-ai-201-migration-guide "Direct link to Spring AI 2.0.1 Migration Guide")
...
The `isInternalToolExecutionEnabled` flag has been removed in Spring AI 2.0.1. To execute tools automatically, route the call through `ChatClient` (which wires in `ToolCallingAdvisor`):
...
`OrchestrationChatOptions` and `DefaultToolCallingChatOptions` are now immutable. Use the builder to set per-request options such as tool callbacks:
