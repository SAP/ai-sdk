...
Before using the Orchestration module, ensure that you have met all the general requirements outlined in the [overview](/ai-sdk/docs/java/next/overview.md#general-requirements).
...
When using a custom resource group, ensure that you have a deployment of orchestration is running in your resource group. Refer to [this guide](https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/orchestration) on how to set up such a deployment.
...
### Create a Client and Choose an LLM[​](#create-a-client-and-choose-an-llm "Direct link to Create a Client and Choose an LLM")
...
Either create the client from scratch as follows.
...
Or use an existing client and add the resource group to it.

```
var existingClient = new OrchestrationClient();

var client = existingClient.withResourceGroup("myResourceGroup","orchestration");
```
...
// You can use this to get the last message in the chat response which is the assistant's reply.

Message lastMessage = result.getLastMessage();
```
...
## Harmonized API[​](#harmonized-api "Direct link to Harmonized API")

The [Harmonized API](https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/harmonized-api?locale=en-US) lets you use different foundation models without the need to change the client code. You can switch from one LLM to another like in the example below.
...
### Prompt Templates from Prompt Registry[​](#prompt-templates-from-prompt-registry "Direct link to Prompt Templates from Prompt Registry")

Alternatively, you can use already prepared templates from the [**Prompt Registry**](/ai-sdk/docs/java/next/ai-core/prompt-registry.md) of SAP AI Core instead of passing a template in the request yourself.

#### Tenant Scope Level Approach[​](#tenant-scope-level-approach "Direct link to Tenant Scope Level Approach")
...
#### Resource Group Scope Level Approach[​](#resource-group-scope-level-approach "Direct link to Resource Group Scope Level Approach")

```
var clientWithResourceGroup = new OrchestrationClient().withResourceGroup("resource-group-name", "scenario-name" );
...
Apply [**input filtering**](https://help.sap.com/docs/sap-ai-core/generative-ai/enhancing-model-consumption-with-input-filtering-75c6d42355924b28905ffe435af93b51) and [**output filtering**](https://help.sap.com/docs/sap-ai-core/generative-ai/enhancing-model-consumption-with-output-filtering-0675b9c77d40467bb2f87b6474e95a80) to the chat completion:
...
**Since v1.9.0** The `promptShield` has been introduced in `AzureContentFilter` for input filtering to safeguard against prompt attacks. **Since v1.16.0** The `protectedMaterialCode` has been introduced in `AzureContentFilter` for output filtering to safeguard against copyrighted content.

### Behavior of Input and Output Filters[​](#behavior-of-input-and-output-filters "Direct link to Behavior of Input and Output Filters")
...
### Custom Masking[​](#custom-masking "Direct link to Custom Masking")
...
## Grounding[​](#grounding "Direct link to Grounding")
...
### Vector Data Repository[​](#vector-data-repository "Direct link to Vector Data Repository")
...
### Grounding via SharePoint[​](#grounding-via-sharepoint "Direct link to Grounding via SharePoint")

Files stored in a SharePoint can be used as the base for the grounding. A guide on how to setup a SharePoint for this can be found in this [SAP wiki](https://wiki.one.int.sap/wiki/pages/viewpage.action?spaceKey=AI\&title=SharePoint+with+client+credential). In order to then use the SharePoint for grounding in your code, you can add its corresponding data repository ID to the `DocumentGroundingFilter` object.
...
### Grounding via *help.sap.com*[​](#grounding-via-helpsapcom "Direct link to grounding-via-helpsapcom")
...
### Mask Grounding[​](#mask-grounding "Direct link to Mask Grounding")
...
### Retrieving Additional Metadata[​](#retrieving-additional-metadata "Direct link to Retrieving Additional Metadata")

To include [metadata](https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/contextualized-retrieval-using-metadata-and-vector-search?locale=en-US) in the grounding result, you can set the `metadataParams` options on the `Grounding` object by providing one or more strings of the metadata keys. If you provide `"*"`, all metadata will be included.
...
Note that retrieving this additional metadata information is not supported for data repository type *help.sap.com*. When using grounding via *help.sap.com* the SDK will ignore the provided metadata parameters.
...
### Asynchronous Streaming[​](#asynchronous-streaming "Direct link to Asynchronous Streaming")
...
Reasoning-capable models can return their intermediate "thinking" alongside the final answer. Enable this by setting the reasoning effort on the model via `withReasoningEffort`, then read the reasoning text from the response with `getReasoningText`.
...
var response = client.chatCompletion(prompt, reasoningConfig);



String answer = response.getContent();

String reasoning = response.getReasoningText();
```

`ReasoningEffort` offers the harmonized levels `MINIMAL`, `LOW`, `MEDIUM`, `HIGH`, and `NONE`. Not all reasoning-capable models support the full range; where a model does not support a given effort, orchestration transparently maps it to the closest equivalent.

### Streaming Reasoning[​](#streaming-reasoning "Direct link to Streaming Reasoning")

When streaming, each delta carries its answer and reasoning fragments separately. Read them with `getDeltaContent` and `getDeltaReasoningText`.
...
try (var stream = client.streamChatCompletionDeltas(request)) {
    ...
    stream.forEach(
        ...
        delta -> {
            ...
            System.out.print(delta.getDeltaReasoningText()); // thinking, as it arrives

            System.out.print(delta.getDeltaContent());          // answer, as it arrives
...
Both methods return an empty string when the delta carries no fragment of that kind.

### Multi-Turn Reasoning[​](#multi-turn-reasoning "Direct link to Multi-Turn Reasoning")

To keep reasoning enabled across turns, feed the previous response's `getAllMessages()` into the follow-up prompt as message history.

```
var firstResponse = client.chatCompletion(new OrchestrationPrompt("What is 6 times 7?"), reasoningConfig);
...
var followUpResponse = client.chatCompletion(followUp, reasoningConfig);



String answer = followUpResponse.getContent();

String reasoning = followUpResponse.getReasoningText();
```
...
### Add Images to a Message[​](#add-images-to-a-message "Direct link to Add Images to a Message")
...
### Add File Attachment to a Message[​](#add-file-attachment-to-a-message "Direct link to Add File Attachment to a Message")

A file can be added to a message either via a file path or via a base64 encoded string as follows. Only PDF files are currently supported for file attachments.

#### Add File via Local File Path[​](#add-file-via-local-file-path "Direct link to Add File via Local File Path")
...
#### Add File via Remote File Path[​](#add-file-via-remote-file-path "Direct link to Add File via Remote File Path")
...
#### Add File via Base64 Encoded String[​](#add-file-via-base64-encoded-string "Direct link to Add File via Base64 Encoded String")
...
### Add Multiple Text Inputs to a Message[​](#add-multiple-text-inputs-to-a-message "Direct link to Add Multiple Text Inputs to a Message")
...
Note, that the LLM will only exactly adhere to the given schema if you use `withStrict(true)` and annotate schema fields with `@JsonProperty(required = true)`. Not all schemas are possible for OpenAI in strict mode. See [here](https://platform.openai.com/docs/guides/structured-outputs#supported-schemas) for more information.
...
Translate the input or output to [any supported language](https://help.sap.com/docs/translation-hub/sap-translation-hub/supported-languages?version=Cloud#translation-provider-sap-machine-translation). The following sample will translate the prompt into English, send it to the LLM, and then translate the output to German. The source language is identified from the whole user input.
...
### Translation using applyTo parameter[​](#translation-using-applyto-parameter "Direct link to Translation using applyTo parameter")

The following sample will translate only the given placeholders into English, send it to the LLM, and then translate the output to German.

Note that setting the source language through `withSourceLanguage()` works slightly different when used together with applyTo parameters.

If either `applyToPlaceholders` or `applyToTemplateRoles` were not used, the source language is set for the whole message. If `applyToPlaceholders` is used however, `withSourceLanguage` applies to the most recently added placeholder only. This is why in the below example the source language of the placeholders was set to German, while the source language of the template roles was set to English.
...
var systemMessage =
    ...
    Message.system("You are an expert study coach creating clear, concise exam notes and practice questions.");

var userMessage =

    Message.user("Generate a study guide for the {{?exam_type}} exam on {{?topic}}.\n\nInclude {{?num_questions}} practice questions.");



var templatingConfig = TemplateConfig.create().withMessages(systemMessage, userMessage);
...
Note that setting `TranslationConfig.applyTo()` to either `null` as shown in first example or to an actual value as shown in the second example is necessary.
...
## Configure the LLM[​](#configure-the-llm "Direct link to Configure the LLM")
...
## Using a prepared Configuration[​](#using-a-prepared-configuration "Direct link to Using a prepared Configuration")

There are different ways to create an Orchestration configuration outside the runtime of your application.

### Using a Configuration stored in Prompt Registry[​](#using-a-configuration-stored-in-prompt-registry "Direct link to Using a Configuration stored in Prompt Registry")

You can create and store Orchestration configurations in Prompt Registry. For a guide on how to do that please refer to the chapter on [Prompt Registry](/ai-sdk/docs/java/next/ai-core/prompt-registry.md#orchestration-configurations-in-prompt-registry).
...
Note that you have to add the necessary templating parameters that are used in your Orchestration configuration. Additionally, you can add a list of messages as message history. Here is an example that uses both for a stored Orchestration configuration that is referenced via its ID.
...
var reference =
    ...
    OrchestrationConfigReference.fromId("0123")
        ...
        .withTemplateParameters(parameter);
...
### Using a Configuration from AI Launchpad[​](#using-a-configuration-from-ai-launchpad "Direct link to Using a Configuration from AI Launchpad")
...
## Using Fallback Modules[​](#using-fallback-modules "Direct link to Using Fallback Modules")

It is possible to provide multiple `OrchestrationModuleConfig` objects to an Orchestration call instead of a single one. The provided configurations are then tried in the order they are given and if a call with one configuration failed, the next in line is used as fallback. For more information, see the [AI Core guide on fallback modules](https://help.sap.com/docs/sap-ai-core/generative-ai/orchestration-with-fallbacks?locale=en-US).

To use this feature, you can simply provide multiple `OrchestrationModuleConfig` objects in your (steaming or non-streaming) call.
...
The Orchestration service can return citations in the response when the LLM provides them, such as with Perplexity's Sonar model.
