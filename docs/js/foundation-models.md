...
Initialize a client with the model name. If the model version is not specified, it is set to `latest`.
...
When initializing a client to use a foundation model, it is equivalent to providing a combination of model name and model version, or a deployment ID to identify a model within a [Resource Group](https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/resource-groups?q=resource+group). If multiple deployments were created with the same model name and version, the first deployment will be used.
...
```
const client = await new AzureOpenAiChatClient('gpt-5', {
...
You can attach custom headers to your request, including SAP internal metering headers (for example, `premium-billing` and `user-context`), using the `requestConfig.headers` property. These internal metering headers are typically used for internal cost allocation and context propagation.
