...
The `@sap-ai-sdk/orchestration` package provides a client for the orchestration service of SAP AI Core. Use the orchestration embedding client to generate vector embeddings from text inputs using a harmonized API across embedding models. You can optionally enable orchestration modules such as [masking](#data-masking) for Personally Identifiable Information (PII) protection.

Find more details about orchestration workflow [here](https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/orchestration-workflow-v2). For more details about embeddings endpoint, refer [here](https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/embeddings). Find more details about Chat Completion [here](/ai-sdk/docs/js/orchestration/chat-completion.md).
...
## Response Object and Numeric Embeddings[​](#response-object-and-numeric-embeddings "Direct link to Response Object and Numeric Embeddings")
...
## Batch Embedding[​](#batch-embedding "Direct link to Batch Embedding")
...
## Custom Deployment Configuration[​](#custom-deployment-configuration "Direct link to Custom Deployment Configuration")
...
The SAP Cloud SDK for AI provides access to the underlying HTTP response through the `response` property on orchestration embedding responses. Use this for advanced scenarios such as accessing response headers, inspecting HTTP status codes, or debugging.
...
console.log(response.getEmbeddings()); // Access embedding vectors



// Advanced: Access HTTP response details when needed

console.log(response.response.status); // HTTP status code

console.log(response.response.headers); // Response headers

console.log(response.response.data); // Raw response data
```

tip

For most use cases, prefer the convenience methods like `getEmbeddings()`. Use `response` only when you need direct access to HTTP-level details.
...
const response = embeddingClient.embed(
...
HTTP request cancellation may not cancel the request on the orchestration service. The cancellation stops the HTTP connection from the client side, but the service may continue processing the request. Metering information may not be available for cancelled requests.
...
See [Error Handling](/ai-sdk/docs/js/error-handling.md) for general guidance.
