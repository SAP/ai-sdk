...
The `@sap-ai-sdk/orchestration` package provides a client for the orchestration service of SAP AI Core. The orchestration service harmonizes the API of various generative AI models, enabling seamless integration and interaction with different models through a unified interface. Additionally, it provides features like [templating](#prompt-templating), [content filtering](#content-filtering), [grounding](#grounding) and more to enhance the interaction with generative AI models.
...
Initialize a client with proper orchestration configuration for each module. Below is an example for configuring the mandatory `promptTemplating` module. In addition, you can find more sample code [here](https://github.com/SAP/ai-sdk-js/blob/main/sample-code/src/orchestration.ts).
...
Here, only the model name is configured in the `promptTemplating` module. The user message is passed directly to the `chatCompletion()` method call.
...
* `getCitations()` returns the list of citations from the response, useful when using models like [Perplexity Sonar](#using-citations-with-perplexity-sonar-models).
* `getIntermediateFailures()` returns information about non-fatal errors that occurred during the request, such as when using [module fallback](#module-fallback).
...
## Prompt Templating[​](#prompt-templating "Direct link to Prompt Templating")
...
The orchestration service supports module fallback, which allows you to configure multiple complete orchestration module configurations that are tried in sequence until one succeeds. This feature is useful for implementing resilience and fallback strategies when working with AI models.

Each configuration in the fallback chain is a complete `OrchestrationModuleConfig`, allowing you to customize all aspects of orchestration for each fallback option, not just the model. This includes [system prompts](#template-configuration), [content filtering](#content-filtering) rules, [data masking](#data-masking) settings, [grounding](#grounding) configurations, and other orchestration module settings. This flexibility accommodates model-specific requirements, such as varying parameters, system prompts, or content filtering rules, enabling tailored fallbacks for different use cases or constraints.

To use module fallback, provide an array of `OrchestrationModuleConfig` objects when initializing the client. The orchestration service will try each configuration in order until one succeeds.
...
const orchestrationClient = new OrchestrationClient([

  {
    ...
    // Primary configuration with strict filtering and detailed prompt
        ...
        template: [

          {
            ...
            content:
              ...
              'You are an expert AI analyst. Provide detailed, comprehensive responses.'
...

  {
    ...
    // Fallback configuration: faster model with relaxed filtering and concise prompt
      ...
      prompt: {
        ...
        template: [

          {
            ...
            content: 'Provide a brief, concise response.'
...
const response = await orchestrationClient.chatCompletion({
  ...
  placeholderValues: {
    ...
    question: 'Write a comprehensive analysis of AI trends.'
...
The `timeout` parameter in the model configuration sets the maximum time (in seconds) for the LLM request. If the request exceeds this timeout in non-streaming scenarios, the orchestration service triggers the fallback to the next configuration. The timeout parameter applies to the individual LLM call, not the entire orchestration request. For more information on timeouts in orchestration, refer to the [Orchestration Retry and Timeout](#orchestration-retry-and-timeout) section.

The `getIntermediateFailures()` method returns information about any failed attempts before a successful response is obtained.
...
When using module fallback with different models, the SAP Cloud SDK for AI logs a warning if duplicate models are detected in the fallback chain. Consider using different models for meaningful fallback behavior.

#### Module Fallback Conditions[​](#module-fallback-conditions "Direct link to Module Fallback Conditions")

Module fallback is triggered under specific conditions, which may change in future versions of the orchestration service.

**For streaming requests:** Fallback happens before the stream starts. The model chosen by the orchestration service will not switch mid-stream.

* The model is not available.

**For non-streaming requests:** Fallback may occur even after the request starts being handled by the first provider. The following additional conditions can trigger fallbacks:

* Request timeout
* Too many requests
* Internal Service errors

Module fallback is **not** triggered for content-filter hits or other errors, such as invalid input errors.

### Harmonized API[​](#harmonized-api "Direct link to Harmonized API")

The [Harmonized API](https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/harmonized-api?locale=en-US) lets you use different foundation models without the need to change the client code. You can switch from one LLM to another easily like the example below.
...
#### Using Citations with Perplexity Sonar Models[​](#using-citations-with-perplexity-sonar-models "Direct link to Using Citations with Perplexity Sonar Models")

Some models provide citations that reference the sources used to generate responses, making them ideal for use cases requiring up-to-date information and source transparency. Citations can be accessed using the `getCitations()` method on the response object. For details on the citation properties, refer to the [API documentation](/ai-sdk/api/v2/types/orchestration_src.Citation.html).

Each citation contains the following properties:

* `ref_id`: Unique numeric identifier for the inline citation
* `title`: Title of the cited source
* `url`: URL of the cited source

warning

Citations are currently only supported by Perplexity Sonar models.

Use the `getCitations()` method to retrieve citations from the response:
...
if (citations) {
  ...
  citations.forEach(citation => {
    ...
    console.log(`[${citation.ref_id}] ${citation.title}: ${citation.url}`);
...
#### Consuming SAP ABAP 1 Model[​](#consuming-sap-abap-1-model "Direct link to Consuming SAP ABAP 1 Model")

SAP-ABAP-1 is a foundation model built by SAP that helps with coding tasks and can be only be consumed through the orchestration service. It is fine-tuned on a large amount of SAP ABAP code and is optimized for explaining ABAP code snippets. When using this model, input and output filtering is configured by default.
...
Template messages are **always prepended** to every request. Reusing the same client across multiple turns will include the template on every call. See the [LangGraph tutorial](/ai-sdk/docs/js/tutorials/langgraph-template-orchestration-client.md) for how to avoid duplication in multi-turn workflows with prompt templates.
...
Messages passed alongside a `template_ref` are **automatically routed to `messages_history`** and cannot be merged into the remotely stored template. See the [LangGraph tutorial](/ai-sdk/docs/js/tutorials/langgraph-template-orchestration-client.md) for the recommended two-client pattern in multi-turn workflows with template references.

A prompt template can be referenced either by ID, or by a combination of name, scenario and version. For details on storing a template in the Prompt Registry, refer to [this guide](https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/create-prompt-template-imperative).

### Scoped Prompt Registry[​](#scoped-prompt-registry "Direct link to Scoped Prompt Registry")
...
Alternatively, you can use the `resource_group` scope to store prompts specific to a particular resource group. This is useful when prompt templates need to be isolated and should not be shared with others in the same tenant.

To reference a scoped prompt template, specify the `scope` property in the `template_ref` alongside either the template `id` or template `name`, `scenario`, and `version`. When using a non-default resource group, make sure to also provide the `resourceGroup` parameter in the client deployment config.
...
### Local Prompt Template[​](#local-prompt-template "Direct link to Local Prompt Template")

You can define the template in a YAML file and pass its content as a string to the `promptTemplating.prompt` property. This is useful for testing a prompt template locally before storing it in the [Prompt Registry](https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/prompt-registry).
...
Define and pass [tool definitions](https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/tool-calling) to enable the model to call specific functions. Here's an example of temperature conversion using tool calls:
...
Set `strict` to `true` to ensure function calls adhere to the function schema. For more information refer to Orchestration [Tool Calling](https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/tool-calling).
...
We recommend using Zod v4 for full compatibility with this package. If you're upgrading from an earlier version, refer to the [Zod v4 migration guide](https://zod.dev/v4/changelog) and pay attention to breaking changes like the switch from `describe('...')` to `meta({ description: '...' })`
...
### Image Recognition[​](#image-recognition "Direct link to Image Recognition")
...
This section covers explicit cache control for AWS Bedrock-hosted models (Anthropic Claude and Amazon Nova). Other model providers, such as OpenAI and Gemini, use implicit context caching that is enabled by default and requires no additional configuration. For a full overview, refer to the [Prompt Caching documentation](https://help.sap.com/docs/sap-ai-core/generative-ai/prompt-caching?locale=en-US).

Cache control improves performance by caching requests at the model provider level. Add a `cache_control` breakpoint to a content block to mark a cache breakpoint in the prompt. Availability of different time-to-live (TTL) options may depend on the model, refer to the [orchestration prompt caching documentation](https://help.sap.com/docs/sap-ai-core/generative-ai/prompt-caching?locale=en-US) for details.

note

Prompt caching requires a minimum number of tokens in the cached content. If the prompt is too short, the cache breakpoint may not take effect.

Set the `cache_control` property as part of message contents. Some models can also apply `cache_control` to tool calls. Use the `getTokenUsage()` method to inspect cache activity. For supported models, the `prompt_tokens_details` property on the tokens usage object will contain the amount of tokens read and written to the cache.
...
The `file` content type can only be used in messages with `role: 'user'`. Attempting to use `file` in non-user messages will result in an error.

For file input, the [content block](/ai-sdk/api/v2/types/orchestration_src.UserChatMessageContentItem.html) should have `type: 'file'` and include a [`file`](/ai-sdk/api/v2/types/orchestration_src.FileContent.html) object with a `file_data` property containing a URL. You can either provide a public HTTPS URL or a data URL with base64 encoded file content.

note

A model's support for file input via the orchestration service may not match the file input capabilities of each individual model when accessed directly. Please always check if the specific file type you want to use is supported via the orchestration service for your chosen model.
...
It is recommended to include a `filename` in the payload; otherwise, the orchestration service may assign one automatically.

The following example reads a local file and sends it to the model. Adjust the file path, MIME type and model name as needed, for PDF files use `application/pdf` as MIME type.
...
const data = await readFile('PATH_TO_FILE', 'base64');

const fileUrl = `data:MIME_TYPE;base64,${data}`;
...
const response = await orchestrationClient.chatCompletion({
  ...
  messages: [

    {
      ...
      role: 'user', // only user role supports file

      content: [

        { type: 'text', text: 'Summarize this document.' },

        { type: 'file', file: { file_data: fileUrl, filename: 'FILENAME' } }
...
The `chatCompletion()` method can throw an error with HTTP status code `400` if content filters hit. In case of a `200` HTTP response, the `getContent()` method can throw an error if the output filters hit. See the [Error Handling](/ai-sdk/docs/js/error-handling.md) page for more details.
...
#### Prompt Attack Detection[​](#prompt-attack-detection "Direct link to Prompt Attack Detection")
...
The Azure Content Safety service also supports prompt attack detection for input text via a prompt shield configuration. Set the `prompt_shield` property to `true` to enable the detection. This option is only available for input filters. For more information, refer to [Prompt Shields in Azure AI Content Safety](https://learn.microsoft.com/en-us/azure/ai-services/content-safety/concepts/jailbreak-detection).
...
#### Protected material detection for code[​](#protected-material-detection-for-code "Direct link to Protected material detection for code")

Protected material code detection identifies and safeguards against the inclusion of proprietary or copyrighted code snippets in AI-generated responses. This scan proactively monitors for protected code snippets that match those in software libraries, source code, algorithms, and other intellectual property sourced from known GitHub repositories, preventing unauthorized use or potential legal issues. To enable protected material code detection, set the `protected_material_code` property to `true` in the Azure content safety input filter configuration. The property is only supported for output filter configurations. For more information, refer to [Azure AI Content Safety Protected Material Detection](https://learn.microsoft.com/en-us/azure/ai-services/content-safety/concepts/protected-material).
...
### SAP Data Privacy Integration[​](#sap-data-privacy-integration "Direct link to SAP Data Privacy Integration")
...
The `allowlist` property specifies terms which will be kept unmasked. Set `mask_grounding_input` to `true` to mask [Grounding](#grounding) input as well.
...
Set `metadata_params` property with an array of parameter names to include [Metadata](https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/contextualized-retrieval-using-metadata-and-vector-search?locale=en-US) in the grounding result, which can be mentioned when writing the prompt. If set the value to `'*'`, all metadata will be included.
...
The module supports SAP's Document Translation service and allows translating both [input](https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/input-translation) and [output](https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/output-translation) of a model. The target language is mandatory, while source language will be auto-detected if not provided.

For available translation options, consult the [list of supported languages](https://help.sap.com/docs/translation-hub/sap-translation-hub/supported-languages-6854bbb1bd824ffebc3a097a7c0fd45d) to identify compatible language pairs. Input and output translation configurations are differentiated. Set the `type` parameter to `input` to build the input configuration and `output` to build the output configuration.
...
### ApplyTo Selector[​](#applyto-selector "Direct link to ApplyTo Selector")

The `applyTo` property in the translation configuration allows you to selectively translate which messages or parts of messages in the conversation should be translated. This provides granular control over translation in multi-turn conversations.
...
##### Template Roles[​](#template-roles "Direct link to Template Roles")
...
#### Automatic Target Language Inference[​](#automatic-target-language-inference "Direct link to Automatic Target Language Inference")

By detecting the source language from translation, the output translation can automatically respond in the user's original language. There is no need to specify a fixed target language, instead reference the `applyTo` item from the input translation.
...
#### Chat History Control[​](#chat-history-control "Direct link to Chat History Control")

By default, chat history messages are translated. If chat history message translation is not necessary, set the `translateMessagesHistory` to `false`.
...
## Use JSON Configuration from AI Launchpad[​](#use-json-configuration-from-ai-launchpad "Direct link to Use JSON Configuration from AI Launchpad")
...
### Streaming with Citations[​](#streaming-with-citations "Direct link to Streaming with Citations")

When using models that support citations, citations are available after the stream completes and can be retrieved using the `getCitations()` method. For more information on the citation properties, refer to the [API documentation](/ai-sdk/api/v2/types/orchestration_src.Citation.html).
...
// Get citations after stream completes
...
Citations may not always be present in the response depending on the query and whether the model used web sources.
...
#### Module Fallback with Streaming[​](#module-fallback-with-streaming "Direct link to Module Fallback with Streaming")

[Module fallback](#module-fallback) is also supported when using the `stream()` method. The service tries each configuration in sequence, and you can access intermediate failures after the stream completes. For details on configuring module fallback, see the [Module Fallback section](#module-fallback).
...
// Check for fallback attempts after streaming completes

const failures = response.getIntermediateFailures();

if (failures) {

  console.log(`Used ${failures.length} fallback(s) before success`);

  failures.forEach((failure, index) => {

    console.log(

      `Failure ${index + 1} (Code: ${failure.code} at ${failure.location}): ${failure.message}`
...
Providing streaming option overrides is only possible when your orchestration configuration uses module fallback with more than one configuration. Overrides cannot change global options like `chunk_size`.

When using streaming with module fallback, you can provide streaming option overrides for each configuration in the fallback chain.

The streaming options structure consists of a `global` object containing options shared across all configurations, such as `chunk_size`. It also includes module-specific options, like `promptTemplating` or `outputFiltering`, that apply to all configurations unless overridden, and an optional `overrides` object that maps configuration indices to option overrides. When an override is provided for a specific configuration index, it replaces the base module-specific options entirely for that configuration. If no override is provided for a configuration, it uses the base options from the top-level properties (everything except `global` and `overrides`). Overrides cannot modify global options or contain additional overrides.
...
const response = await orchestrationClient.stream(
...

  {
    ...
    promptTemplating: { include_usage: false }, // Config inherited by all modules if not overriden.

    // Module-specific overrides

    overrides: {

      // Config 0 replaces base options fully.

      0: { promptTemplating: { include_usage: true } },

      // Config 1 uses base options because no override is provided.

      // Config 2 replaces base options fully.

      2: { outputFiltering: { overlap: 50 } }
...
## Initialization with Orchestration Configuration[​](#initialization-with-orchestration-configuration "Direct link to Initialization with Orchestration Configuration")

Instead of defining the orchestration configuration inline, you can reference a stored configuration from the prompt registry. This approach enables configuration reuse across applications and centralized management of orchestration settings.

### By Configuration ID[​](#by-configuration-id "Direct link to By Configuration ID")
...
### By Scenario, Name, and Version[​](#by-scenario-name-and-version "Direct link to By Scenario, Name, and Version")
...
Messages passed alongside a configuration reference are **automatically routed to `messages_history`** and cannot be merged into the remotely stored configuration's prompt template. See the [LangGraph tutorial](/ai-sdk/docs/js/tutorials/langgraph-template-orchestration-client.md) for the recommended two-client pattern in multi-turn workflows with configuration references.
...
Use a custom request configuration to attach SAP internal metering headers to enable internal cost allocation and context propagation. Use `premium-billing` to indicate the billing plan or token (e.g., "premium-plan") and `user-context` to pass caller metadata.
...
const response = await orchestrationClient.chatCompletion(
...

  {
    ...
    headers: {
      ...
      'user-context': 'userId=12345;role=developer' // or a JSON/base64 string depending on your internal convention
...
For most use cases, prefer the convenience methods like `getContent()`, `getMessages()`, and `getTokenUsage()`. Use `rawResponse` only when you need direct access to HTTP-level details.

The `rawResponse` property is available on chat completion and streaming response objects. For embedding responses, access the raw HTTP response through the `response` property.
...
### Orchestration Retry and Timeout[​](#orchestration-retry-and-timeout "Direct link to Orchestration Retry and Timeout")

info

Models provided by GCP Gemini currently do not support the Orchestration Service `timeout` and `max_retries` options.

The orchestration service supports built-in retry and timeout configurations on the model configuration. These can be configured when initializing the client.
...
### SAP Cloud SDK Resilience Package[​](#sap-cloud-sdk-resilience-package "Direct link to SAP Cloud SDK Resilience Package")

info

Some orchestration service resilience features (timeout and retry) are also enabled by default.

Use the `resilience()` function from `@sap-cloud-sdk/resilience` to add SDK-level resilience. By default, it enables a circuit breaker and a 10-second timeout.
