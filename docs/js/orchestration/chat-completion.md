...
Messages with Prompt Template References

When using the `template_ref` property, messages passed to the `chatCompletion()` or `stream()` methods are automatically appended to the `messages_history` array as they cannot be merged into the stored prompt template. The `messages` property defined in the `template` array are appended after any existing `messagesHistory` entries.
...
Messages with Orchestration Configuration References
