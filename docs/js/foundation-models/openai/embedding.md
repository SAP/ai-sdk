...
Initialize the `AzureOpenAiEmbeddingClient` by following the instructions in the [Client Initialization](/ai-sdk/docs/js/foundation-models.md#client-initialization) section.
...
For most use cases, prefer the convenience methods like `getEmbedding();`. Use `rawResponse` only when you need direct access to HTTP-level details.
