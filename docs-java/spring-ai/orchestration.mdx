...
You can use the `OrchestrationSpringAiEmbeddingModel` to generate embeddings via Orchestration.
Simply provide the the model name and the input text as follows:

```java
EmbeddingOptions options = EmbeddingOptionsBuilder.builder()
    .withModel("text-embedding-3-small").build();

var model = new OrchestrationSpringAiEmbeddingModel(options);
