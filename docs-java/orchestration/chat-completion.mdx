...
Before using the Orchestration module, ensure that you have met all the general requirements outlined in the [overview](../overview.mdx#general-requirements).
...
var config = new OrchestrationModuleConfig(OrchestrationAiModel.CLAUDE_3_5_SONNET);
```
...
Alternatively, you can use already prepared templates from the [**Prompt Registry**](../ai-core/prompt-registry.mdx) of SAP AI Core instead of passing a template in the request yourself.
...
Reasoning-capable models can return their intermediate "thinking" alongside the final answer.
Enable this by setting the reasoning effort on the model via `withReasoningEffort`, then read the reasoning text from the response with `getReasoningText`.
...
`ReasoningEffort` offers the harmonized levels `MINIMAL`, `LOW`, `MEDIUM`, `HIGH`, and `NONE`.
Not all reasoning-capable models support the full range; where a model does not support a given effort, orchestration transparently maps it to the closest equivalent.

### Streaming Reasoning

When streaming, each delta carries its answer and reasoning fragments separately.
Read them with `getDeltaContent` and `getDeltaReasoningText`.
...
For a guide on how to do that please refer to the chapter on [Prompt Registry](../ai-core/prompt-registry.mdx#orchestration-configurations-in-prompt-registry).
...
var workingConfig = new OrchestrationModuleConfig(GPT_4O_MINI);
var secondWorkingConfig = new OrchestrationModuleConfig(GPT_4O);
var brokenConfig =
