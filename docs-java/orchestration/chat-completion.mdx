...
The following sample will translate the whole prompt into English, send it to the LLM, and then translate the output to German.
...
### Translation using apply_to parameter
...
var configWithTranslation = config
        ...
        .withInputTranslationConfig(TranslationConfig.translateInputTo("en-US").applyToPlaceholders("exam_type", "topic").withSourceLanguage("de-DE"))
        .withOutputTranslationConfig(TranslationConfig.translateOutputTo("de-DE").withSourceLanguage("en-US"));
...
Note that setting `SAPDocumentTranslationInputConfig.applyTo()` to either `null` as shown in first example or to an actual value as shown in the second example is necessary.
