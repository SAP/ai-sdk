...
var filterStrict = new AzureContentFilter()
                ...
                .violence(ALLOW_SAFE)
                .promptShield(true);
var filterStrictOutput = filterStrict.protectedMaterialCode(true);
...
var filterLooseOutput = filterLoose.protectedMaterialCode(true);
...
var configWithFilter = config.withInputFiltering(filterStrict).withOutputFiltering(filterStrictOutput, llamaGuardFilter);
...
**Since v1.16.0** The `protectedMaterialCode` has been introduced in `AzureContentFilter` for output filtering to safeguard against unauthorized code disclosure.
