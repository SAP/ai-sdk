...
String reasoning = response.getReasoningText(); // empty if the model returned no reasoning
```
...
Read them with `getDeltaContent` and `getDeltaReasoningContent`.
...
try (var stream = client.streamChatCompletionDeltas(request)) {
    stream.forEach(
        delta -> {
            System.out.print(delta.getDeltaReasoningContent()); // thinking, as it arrives
...
String reasoning = followUpResponse.getReasoningText(); // empty if the model returned no reasoning
```
