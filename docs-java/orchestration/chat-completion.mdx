...
It's possible to add images, PDF files, and multiple text inputs to a message.
...
### Add PDF File Attachment to a Message

A PDF file can be added to a message either via a file path or via a base64 encoded string as follows.

#### Add PDF via File Path

```java
var message = Message.user("What is the title of the topic discussed here?");
var newMessage = message.withPdf("path/to/sample.pdf");
```

#### Add PDF via Base64 Encoded String

```java
var message = Message.user("What is the title of the topic discussed here?");
var newMessage = message.withPdfBase64(base64Data, "sample.pdf");
```

Note, that currently only user messages are supported for PDF file attachments.
