...
**Since v1.16.0** The `protectedMaterialCode` has been introduced in `AzureContentFilter` for output filtering to safeguard against copyrighted content.
...
### Translation using applyTo parameter
...
If either `applyToPlaceholders` or `applyToTemplateRoles` were not used, the source language is set for the whole message.
If `applyToPlaceholders` is used however, `withSourceLanguage` applies to the most recently added placeholder only.
This is why in the below example the source language of the placeholders was set to German, while the source language of the template roles was set to English.
...
It is possible to provide multiple `OrchestrationModuleConfig` objects to an Orchestration call instead of a single one.
The provided configurations are then tried in the order they are given and if a call with one configuration failed, the next in line is used as fallback.
For more information, see the [AI Core guide on fallback modules](https://help.sap.com/docs/sap-ai-core/generative-ai/orchestration-with-fallbacks?locale=en-US).
...
The Orchestration service can return citations in the response when the LLM provides them,
such as with Perplexity's Sonar model.
