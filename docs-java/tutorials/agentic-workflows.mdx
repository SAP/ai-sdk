...
For technical prerequisites please refer to our [general requirements](../overview.mdx#general-requirements).

## Create Mock Tools

We first create two mock tools.
For more information on tool usage, we refer to the respective [chapter in our documentation](../spring-ai/orchestration.mdx#tool-calling).
...
public class SpringAiAgenticWorkflowService {
  ...
  private final OrchestrationModuleConfig config =
      new OrchestrationModuleConfig(GPT_4O_MINI);
...
For more information on how to use chat memory, we refer to the respective [chapter in our documentation](../spring-ai/orchestration.mdx#chat-memory).
