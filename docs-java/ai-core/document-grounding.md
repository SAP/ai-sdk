...
Note that it is currently required to add the `.matchMode()` parameter to the `VectorDocumentKeyValueListPair`.
Otherwise, the grounding service will return a `500 Internal Server Error`.
