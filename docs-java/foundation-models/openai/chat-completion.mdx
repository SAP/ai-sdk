...
var request = new OpenAiChatCompletionRequest(messages).withToolsExecutable(tools);
var response = client.chatCompletion(request);
var assistantMessage = response.getMessage();

var toolCallsInput = assistantMessage.toolCalls(); // raw tool call input arguments
```
...
var toolResults = OpenAiTool.execute(tools, assistantMessage); 

// 4. Send back the results, and the model will incorporate them into its final response.
messages.add(assistantMessage);
messages.add(toolResults.getMessages());

var finalResponse = client.chatCompletion(request.withMessages(messages));
