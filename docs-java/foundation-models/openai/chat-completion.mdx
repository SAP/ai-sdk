...
var tool = new OpenAiFunctionTool("weather", WeatherMethod.Request.class)
    .withDescription("Get the weather for the given location");

// 2. Assistant calls the function
var request = new OpenAiChatCompletionRequest(messages).withOpenAiTools(List.of(tool));
...
WeatherMethod.Request arguments = functionCall.getArgumentsAsObject(weatherFunction);
