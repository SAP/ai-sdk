...
Below is an example of a tool named `WeatherMethod`, which calculates a response based on the location and temperature unit provided.
...
The first call lets the LLM extract structured arguments from natural language.
You can use this directly in your application.
The second call is optional and lets the LLM turn the tool’s result into a user-friendly response.
