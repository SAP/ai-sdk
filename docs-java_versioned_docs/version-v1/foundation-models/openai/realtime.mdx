...
Before using the AI Core module, ensure that you have met all the general requirements outlined in the [General Requirements](../../overview.mdx#general-requirements).
...
- **A Deployed OpenAI Model in SAP AI Core**
  ...
  - In case the model is deployed in a custom resource group, refer to [this section](chat-completion.mdx#using-a-custom-resource-group).
  - <details>
...
Default system prompt makes model voice (repeat) input. This behavior can be overwritten,
providing a custom system prompt via RealtimeParamSystemPrompt on client creation.
