...
For detailed information on the behaviour of input and output filters, check the [Filter Documentation for Orchestration](../orchestration/chat-completion.mdx#filtering).
...
To use this feature, set the fallback configs on your `OrchestrationChatOptions`:
...
var workingConfig = new OrchestrationModuleConfig().withLlmConfig(GPT_4O);

var opts = new OrchestrationChatOptions(brokenConfig);
opts.setFallbackConfigs(List.of(workingConfig));

Prompt prompt = new Prompt("Why is 'Hello World' so famous?", opts);
ChatResponse response = client.call(prompt);
```
...
Fallback configs work for both streaming and non-streaming calls.

Please find [a working example in our Spring Boot application](https://github.com/SAP/ai-sdk-java/tree/main/sample-code/spring-app/src/main/java/com/sap/ai/sdk/app/services/SpringAiOrchestrationService.java).
...
For detailed information on setting a response format, check the corresponding [Documentation for Orchestration](../orchestration/chat-completion.mdx#response-format).
...
For more information on prompt registry, we refer to the corresponding [documentation](../ai-core/prompt-registry.mdx).

### Prompt Registry with input parameters and chat memory

Please see this [documentation](../ai-core/prompt-registry.mdx#using-templates-in-springai).
