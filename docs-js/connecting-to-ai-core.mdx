...
If no service binding could be found (and no other destination has been given), the SAP Cloud SDK for AI will throw an exception.
...
Then run your application with the hybrid profile:
...
Then set the environment variable `AICORE_SERVICE_KEY`, e.g., by creating a `.env` file with the following content:
...
For NextJS, please see [this documentation](https://nextjs.org/docs/pages/building-your-application/configuring/environment-variables).

## Using a Destination

### Using the BTP Destination Service
...
Ensure you have created an instance of the BTP Destination Service and bound it to your application.
