...
When connecting through a service binding or the `AICORE_SERVICE_KEY` environment variable, the SDK sets a default HTTP socket timeout of **12 minutes** (`720000` ms) for AI Core requests.
...
:::note
The 12-minute default applies only to the SDK-managed service binding destination.
