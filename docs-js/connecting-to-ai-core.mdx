...
- **Per request** — pass a custom [`https.Agent`](https://nodejs.org/api/https.html#class-httpsagent) through the `httpsAgent` property of the request configuration:
  ...
  const response = await orchestrationClient.chatCompletion(request, {
    httpsAgent: new Agent({ timeout: 300000, keepAlive: false })
  });
  ```

- **Per destination** — when [creating a custom destination](#creating-a-custom-destination), set `agentOptions` on it to control the timeout and keep-alive.
