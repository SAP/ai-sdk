...
Use `dtype: 'string'` for identifier-like values such as IDs, postal codes, or category codes, even when they look numeric.
This preserves exact values and avoids interpolation.
...
- **Provide well-structured data**: Use clean, well-structured data with no gaps.
  Include only relevant columns for the prediction task to help the model focus on meaningful patterns.
- **Use clear column metadata**: Use descriptive column names shorter than 100 characters.
  Keep column descriptions at or below 500 characters when you include them in the schema.
- **Keep data types consistent**: Use a consistent data type within each column so the model can infer patterns reliably.
  Consider setting `parse_data_types` to `false` when needed to prevent automatic type conversion.
- **Provide sufficient context**: Use the recommended context length from [SAP Help Portal](https://help.sap.com/docs/sap-ai-core/generative-ai-hub/sap-rpt-1?locale=en-US#tabular-in-context-learning) when preparing input data.
  Providing enough context rows helps the model learn patterns and make more accurate predictions.
...
- **Use string types for identifier-like predictions**: In the provided schema, set identifier-like numeric and categorical columns to `dtype: 'string'` to avoid interpolation.
  This preserves exact values for columns like IDs, postal codes, or category codes.
- **Understand string output constraints**: For columns with `dtype: 'string'`, prediction results can only contain values that already exist in the input data.
  The model treats strings as categorical values and cannot generate new string values that are not present in the input.
