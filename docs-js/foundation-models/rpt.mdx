...
:::important
If you want to assign the schema and prediction data as variables you have to initialize the schema with `as const` and use the generic `PredictionData<typeof schema>` type.
It works without type assertions when passing the data directly to the `predict()` method.
Example:
