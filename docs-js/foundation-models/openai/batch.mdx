...
Prepare an input file in **JSONL format** (one JSON object per line) and upload it to an S3-compatible object store [registered as a secret](https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/register-your-object-store-secret) in SAP AI Core.
...
For example, if `output.uri` is `ai://s3secret/output/`, the file will be at `ai://s3secret/output/{batchId}/output.jsonl`.
