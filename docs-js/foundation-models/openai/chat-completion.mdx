...
const finalResponse = await client.run({
  messages, //A list of the model's previous responses and the function result
  tools: [convertTemperatureTool]
});
...
When appropriate, it will make tool calls to help generate more accurate responses.
