...
While you can also call the `getDeltaToolCalls()` method on individual chunks, partial tool calls are typically not useful.
Therefore, we recommend using the `getToolCalls()` method on the full response instead.
