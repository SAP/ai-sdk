...
The [`OrchestrationClient`](/api/v2/classes/langchain_src.OrchestrationClient.html) supports module fallback, which allows you to provide multiple orchestration configurations that are tried in sequence until one succeeds.
Pass a non-empty array of [`LangChainOrchestrationModuleConfig`](/api/v2/types/langchain_src.LangChainOrchestrationModuleConfig.html) objects when initializing the client, using the [`LangChainOrchestrationModuleConfigList`](/api/v2/types/langchain_src.LangChainOrchestrationModuleConfigList.html) type.
