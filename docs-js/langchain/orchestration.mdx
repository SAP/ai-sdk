...
By default, streaming is available via the `stream()` method, but the `invoke()` method does not use streaming internally.
...
When auto-streaming is enabled, the `invoke()` method uses streaming internally to fetch responses, even though it returns the complete result.
...
Set the `disableStreaming` option to completely disable all streaming functionality:
