...
If you do not need OpenAI-specific features such as the Responses API or structured output via Zod, consider using the [`@sap-ai-sdk/orchestration`](../orchestration/chat-completion.mdx) package instead, which provides a stable interface for chat completion and other AI tasks on SAP AI Core.
:::
...
If you need a stable Azure OpenAI integration without OpenAI-specific features, consider using the [`@sap-ai-sdk/foundation-models`](../foundation-models/foundation-models.mdx) package instead.
