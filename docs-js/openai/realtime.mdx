...
`SapOpenAiRealtime` connects to the OpenAI Realtime API over WebSocket, pre-configured for SAP AI Core.
...
Import `SapOpenAiRealtime` from the `@sap-ai-sdk/openai/realtime` sub-path export and call `createClient()`.
...
const clientWithNameVersion = await SapOpenAiRealtime.createClient({
  deployment: { modelName: 'gpt-realtime', modelVersion: 'latest' }
});

// By deployment ID
const clientWithDeploymentId = await SapOpenAiRealtime.createClient({
