...
:::note
When performing resource-group scoped operations or accessing internal resources, you may want to include SAP internal metering headers such as `premium-billing` and `user-context`.
You can attach these per request via the optional `requestConfig.headers` when calling `.execute()` function.
See the [Custom Headers](../foundation-models#custom-headers) section on the Foundation Models page for a concrete example.
:::
