...
```ts twoslash
import { OrchestrationEmbeddingClient } from '@sap-ai-sdk/orchestration';
const response = await new OrchestrationEmbeddingClient({
...
If the provider returns numeric vectors, each element in `embeddings` is a `number[]`.
...
The array returned by `getEmbeddings()` includes an `index` field.
...
declare const orchestrationConfig: ConstructorParameters<
  typeof OrchestrationEmbeddingClient
>[0];
// ---cut---
...
The SAP Cloud SDK for AI provides access to the underlying HTTP response through the `response` property on orchestration embedding responses.
...
:::tip
For most use cases, prefer the convenience methods like `getEmbeddings()`.
Use `response` only when you need direct access to HTTP-level details.
:::
...
:::info
HTTP request cancellation may not cancel the request on the orchestration service.
...
```ts twoslash
import { OrchestrationEmbeddingClient } from '@sap-ai-sdk/orchestration';
declare const orchestrationEmbeddingConfig: ConstructorParameters<
  typeof OrchestrationEmbeddingClient
>[0];
