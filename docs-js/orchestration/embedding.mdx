...
```ts
import { OrchestrationEmbeddingClient, buildDpiMaskingProvider } from '@sap-ai-sdk/orchestration';
...
const res = await client.embed({
  input: 'Hello, my name is Alice Johnson and my email is alice.johnson@company.com.',
  type: 'text'
});
...
async function embedAndStore(inputs: string[]): Promise<VectorRow[]> {
  ...
  return vectors.map(v => ({ id: String(v.index), vector: v.embedding as number[] }));
}
```
