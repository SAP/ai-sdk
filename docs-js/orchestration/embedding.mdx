...
Access number vector embeddings as follows:
...
- If your provider returns base64 strings instead, decode them according to the provider’s format or request numeric vectors via `embeddings.model.params` when supported (for example, `encoding_format: 'float'`).
