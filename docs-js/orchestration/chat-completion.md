...
The orchestration service supports module fallback, which allows you to configure multiple complete orchestration module configurations that are tried in sequence until one succeeds.
This feature is useful for implementing resilience and fallback strategies when working with AI models.

Each configuration in the fallback chain is a complete `OrchestrationModuleConfig`, allowing you to customize all aspects of orchestration for each fallback option, not just the model.
This includes [system prompts](#template-configuration), [content filtering](#content-filtering) rules, [data masking](#data-masking) settings, [grounding](#grounding) configurations, and other orchestration module settings.
This flexibility accommodates model-specific requirements, such as varying parameters, system prompts, or content filtering rules, enabling tailored fallbacks for different use cases or constraints.

To use module fallback, provide an array of `OrchestrationModuleConfig` objects when initializing the client.
...
The `timeout` parameter in the model configuration sets the maximum time (in seconds) for the LLM request.
If the request exceeds this timeout in non-streaming scenarios, the orchestration service triggers the fallback to the next configuration.
The timeout parameter applies to the individual LLM call, not the entire orchestration request.
For more information on timeouts in orchestration, refer to the [Orchestration Retry and Timeout](#orchestration-retry-and-timeout) section.
:::
...
const response = await orchestrationClient.chatCompletion({ messages: [] });
// ---cut---
...
#### Module Fallback Conditions

Module fallback is triggered under specific conditions, which may change in future versions of the orchestration service.

**For streaming requests:** Fallback happens before the stream starts.
The model chosen by the orchestration service will not switch mid-stream.

- The model is not available.

**For non-streaming requests:**
Fallback may occur even after the request starts being handled by the first provider.
The following additional conditions can trigger fallbacks:
...
#### Using Citations with Perplexity Sonar Models

Some models provide citations that reference the sources used to generate responses, making them ideal for use cases requiring up-to-date information and source transparency.
Citations can be accessed using the `getCitations()` method on the response object.
For details on the citation properties, refer to the [API documentation](/api/v2/types/orchestration_src.Citation.html).
...
:::info Messages with Prompt Template References
...
declare const convertTemperatureTool: ChatCompletionTool;
// ---cut---
...
declare const response: Awaited<
  ReturnType<OrchestrationClient['chatCompletion']>
>;
...
declare const response: Awaited<
  ReturnType<OrchestrationClient['chatCompletion']>
>;
declare const toolMessage: ToolChatMessage;
// ---cut---
...
```ts twoslash
import { PromptTemplatingModule } from '@sap-ai-sdk/orchestration';
// ---cut---
...
For file input, the [content block](/api/v2/types/orchestration_src.UserChatMessageContentItem.html) should have `type: 'file'` and include a [`file`](/api/v2/types/orchestration_src.FileContent.html) object with a `file_data` property containing a URL.
You can either provide a public HTTPS URL or a data URL with base64 encoded file content.

:::info

A model's support for file input via the orchestration service may not match the file input capabilities of each individual model when accessed directly.
Please always check if the specific file type you want to use is supported via the orchestration service for your chosen model.
...
The following example reads a local file and sends it to the model.
Adjust the file path, MIME type and model name as needed, for PDF files use `application/pdf` as MIME type.
...
import {
  OrchestrationClient,
  buildAzureContentSafetyFilter as buildSomeContentSafetyFilter
} from '@sap-ai-sdk/orchestration';
// ---cut---
...
import {
  OrchestrationClient,
  buildDpiMaskingProvider as buildSomeMaskingProvider
} from '@sap-ai-sdk/orchestration';
// ---cut---
...
const orchestrationClient = new OrchestrationClient({
  promptTemplating: {
    model: { name: 'gpt-5' },
    prompt: { template: [{ role: 'user', content: '{{?country}}' }] }
  }
});
// ---cut---
...
const response = await orchestrationClient.stream({ messages: [] });
// ---cut---
...
When using models that support citations, citations are available after the stream completes and can be retrieved using the `getCitations()` method.
For more information on the citation properties, refer to the [API documentation](/api/v2/types/orchestration_src.Citation.html).

```ts twoslash
import { OrchestrationClient } from '@sap-ai-sdk/orchestration';
const orchestrationClient = new OrchestrationClient({
  promptTemplating: { model: { name: 'sonar' } }
});
// ---cut---
const response = await orchestrationClient.stream({
  messages: [
...
declare const controller: AbortSignal;
// ---cut---
...
#### Module Fallback with Streaming

[Module fallback](#module-fallback) is also supported when using the `stream()` method.
The service tries each configuration in sequence, and you can access intermediate failures after the stream completes.
For details on configuring module fallback, see the [Module Fallback section](#module-fallback).
...
:::info
Providing streaming option overrides is only possible when your orchestration configuration uses module fallback with more than one configuration.
Overrides cannot change global options like `chunk_size`.
:::
...
The streaming options structure consists of a `global` object containing options shared across all configurations, such as `chunk_size`.
It also includes module-specific options, like `promptTemplating` or `outputFiltering`, that apply to all configurations unless overridden, and an optional `overrides` object that maps configuration indices to option overrides.
When an override is provided for a specific configuration index, it replaces the base module-specific options entirely for that configuration.
If no override is provided for a configuration, it uses the base options from the top-level properties (everything except `global` and `overrides`).
Overrides cannot modify global options or contain additional overrides.
...
:::info Messages with Orchestration Configuration References
...
declare const orchestrationConfig: OrchestrationModuleConfig;
// ---cut---
...
Use a custom request configuration to attach SAP internal metering headers to enable internal cost allocation and context propagation.
Use `premium-billing` to indicate the billing plan or token (e.g., "premium-plan") and `user-context` to pass caller metadata.
...
:::tip
For most use cases, prefer the convenience methods like `getContent()`, `getMessages()`, and `getTokenUsage()`.
Use `rawResponse` only when you need direct access to HTTP-level details.
:::

The `rawResponse` property is available on chat completion and streaming response objects.
For embedding responses, access the raw HTTP response through the `response` property.
...
### Orchestration Retry and Timeout
...
The orchestration service supports built-in retry and timeout configurations on the model configuration.
...
### SAP Cloud SDK Resilience Package
...
Use the `resilience()` function from `@sap-cloud-sdk/resilience` to add SDK-level resilience.
