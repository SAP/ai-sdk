...
title: Getting Started with Agents using AzureOpenAiChatClient
sidebar_label: Getting Started with Agents
description: Building a Travel Itinerary Assistant with LangGraph and AzureOpenAiChatClient
keywords: 
 - tutorial
 - agents
 - langgraph
...
In this tutorial, you'll build a simple travel assistant agent using the `AzureOpenAiChatClient` from the `@sap-ai-sdk/langchain` package alongside `LangGraph`.
...
```
:::note
Using `MemorySaver` allows sharing context across multiple interactions in a single conversational thread (`conv-1`).
