...
In this tutorial, you'll build a simple travel assistant agent using the LangChain Orchestration Client (`OrchestrationClient`) from the `@sap-ai-sdk/langchain` package alongside `LangGraph`.
...
Initialize the `OrchestrationClient` and add tools using the `buildTools()` method.
