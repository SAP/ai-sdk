...
Initialize the `AzureOpenAiChatClient` and add tools using the `bindTools()` method.
...
Provide this value by invoking the graph with a `new Command({ resume: <follow_up_question>" })` input:
