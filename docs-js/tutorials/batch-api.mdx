...
const response =
  await BatchesApi.batchServiceControllerBatchControllerCreateBatch({
    type: 'llm-native',
...
await retry(
  async () => {
    const { current_status } =
      await BatchesApi.batchServiceControllerBatchControllerGetBatchStatus(
        response.id
      )
        .addCustomHeaders({ 'AI-Resource-Group': 'MY_RESOURCE_GROUP' })
        .execute();
...
```ts
await BatchesApi.batchServiceControllerBatchControllerCancelBatch(batchId)
  .addCustomHeaders({ 'AI-Resource-Group': 'MY_RESOURCE_GROUP' })
  .execute();
```
...
```ts
await BatchesApi.batchServiceControllerBatchControllerDeleteBatch(batchId)
  .addCustomHeaders({ 'AI-Resource-Group': 'MY_RESOURCE_GROUP' })
  .execute();
```
